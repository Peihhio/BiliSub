...
### 3. � 两种运行模式（支持并发）
- **自建 HTTPS 模式（推荐）**：如果你有 HTTPS 域名/公网 IP，系统将直接生成直链提交给阿里云，**支持高并发多任务同时提取**，用于对视频合集进行字幕提取，稳定且高速。
- **第三方直链模式（救急）**：如果没有公网环境，系统内置了多个第三方直链服务作为中转方案。虽受限于单任务排队处理，但**无需任何配置开箱即用**。
...
1.  **HTTPS vs HTTP (并发的关键)**
    ...
    - **HTTP 环境**：自动降级使用内置的第三方直链服务中转，音频上传和转写必须**排队**（单线程），适合个人低频使用。
