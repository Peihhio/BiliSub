...
BiliSub 采用**智能动态双模提取策略**，根据你的配置自动选择最优路线：
- **✅ API Key + Cookie 均有效（推荐）**：优先秒级提取自带字幕（0成本）；无字幕视频自动调用 Paraformer 进行语音转写。
- **⚠️ 仅 API Key 有效**：跳过 B 站字幕提取（B站安全策略，无cookie不返回正确字幕），直接强制对所有视频进行 Paraformer 语音转写，保证提取成功率。
- **⚠️ 仅 Cookie 有效**：仅提取 B 站自带字幕（CC/硬字幕），无字幕的视频将直接标记失败（不消耗 API 额度）。
- **成本估算**：语音转写仅需约 **0.3 元/小时**（视频时长），充值 1 元即可处理约 3 小时的高质量无字幕视频内容。
- **视频合集提取**：支持一键提取整个合集，智能调度并发任务。
...
- **本地直链模式（推荐）**：
  ...
  - **支持 8 并发多任务同时提取**，稳定且高速
  
- **第三方直链模式（开箱即用）** ⭐️ **NEW**：
  - **智能并发探测**：同时检测多个第三方存储服务的可用性
...
| **访客** | `guest` | `guest` | 无提取数量限制，所有访客共享 5 并发，不保存历史记录 |
...
1.  **阿里云 DashScope API Key** ([点击申请](https://dashscope.console.aliyun.com/))
    *   **核心功能**：用于语音转写 (Paraformer) 和 AI 总结 (通义千问)。
    *   **联动机制**：如果未单独配置大模型 API Key，系统默认复用此 Key。**若此 Key 无效或被清空，大模型功能将同步失效**（状态灯变红）。
    *   建议预充值少许余额（如 1-5 元）。
2.  **B站 Cookie (SESSDATA)**
    *   **重要**：B 站对未登录用户的 API 调用有限制，且部分自带字幕需要登录才能正确获取。
    *   **智能状态反馈**：界面会实时显示 Cookie 的有效性。若 Cookie 失效，系统会自动降级策略（需配置 API Key），改为全量语音转写。
3.  **配置本地直链服务 (可选但推荐)**
    *   如果你拥有公网 IP 或域名（支持 HTTP 和 HTTPS），强烈建议在设置中开启并配置。
    *   这将绕过第三方直链服务的不稳定性，实现极速转录。
    *   **界面提示**：输入框下方的动态提示条会明确告知当前是"本地直链（高速）"还是"第三方直链（慢速）"、可否"直接提取"、可否"语音转写"，自己的开销自己掌握。
...
- **Guest 账户限制**：内置 Guest 访客模式，所有访客共享 5 并发槽位（超过则排队），便于分享给朋友体验。访客的历史记录仅存于浏览器会话，刷新即消失。
...
1.  **运行模式对比**
    - **本地直链（推荐）**：支持 HTTP 和 HTTPS，系统会开启直链模式，支持**8 并发**提取，速度极快，直接连接阿里云。
    - **第三方直链模式**：使用内置的第三方直链服务中转，现已支持**5 并发**处理和**智能并发上传**，速度大幅提升，适合个人使用，但仍较本地直链模式慢很多。
