...
### 📺 演示 (Demo)
*(此处建议放一张演示 GIF，展示：配置 -> 提取 -> 结果 的流畅过程)*
![BiliSub Demo](https://via.placeholder.com/800x450.png?text=BiliSub+Demo+Video)
...
### 3. � 两种运行模式（支持并发）
- **自建 HTTPS 模式（推荐）**：如果你有 HTTPS 域名/公网 IP，系统将直接生成直链提交给阿里云，**支持高并发多任务同时提取**，稳定且高速。
- **第三方直链模式（救急）**：如果没有公网环境，系统内置了多个第三方图床/文件床作为中转方案。虽受限于单任务排队处理，但**无需任何配置开箱即用**。
...
| 角色 | 用户名 | 密码 | 权限 |
| :--- | :--- | :--- | :--- |
| **管理员** | `admin` | `admin` | 拥有所有权限，可管理用户 |
| **访客** | `guest` | `guest` | 每日限提取 5 条，适合公开演示 |
...
2.  **B站 Cookie (SESSDATA)**
    ...
    *   配置 Cookie 后，提取的准确率和成功率将大幅提升。
3.  **配置自建 HTTPS 服务 (可选但推荐)**
    *   如果你拥有公网 IP 或域名，强烈建议在设置中开启并配置 HTTPS。
...
- **Guest 账户限制**：内置 Guest 访客模式（每日限额5条），便于分享给朋友体验而不被滥用。

---

## ⚠️ 注意事项 (Caveats)

1.  **HTTPS vs HTTP (并发的关键)**
    - 程序会自动检测当前协议。
    - **自建 HTTPS（推荐）**：系统会开启直链模式，支持**高并发**（多线程）提取，速度极快，直接连接阿里云。
    - **HTTP 环境**：自动降级使用内置的第三方图床中转，音频上传和转写必须**排队**（单线程），适合个人低频使用。
...
*Made with ❤️ by BiliSub Community*
